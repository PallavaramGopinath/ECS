...
{
    public interface IReportsAccountHandler
    {
        Task<List<rptLedgerNameList>> GetLedgerNameList(string brCode);
        Task<List<rptDayBook2>> GetChittaBook(DateTime fromDate, DateTime toDate, decimal yrId,string brCode);
        Task<List<rptDayBook2>> GetDayBook(DateTime fromDate, DateTime toDate, decimal yrId,string brCode);
