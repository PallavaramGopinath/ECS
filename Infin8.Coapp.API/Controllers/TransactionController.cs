...
namespace Infin8.Coapp.API.Controllers
{
    ...
    public class TransactionController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("GetSuspenseAccounts/{suspenseType:int}")]
        public async Task<ActionResult<List<DropdownItem>>> GetSuspenseLedgerItems(int suspenseType)
        {
            List<DropdownItem> ledgers = new List<DropdownItem>();
            ledgers = await _transactionHandler.GetSuspenseLedgerItems(suspenseType);
            if(ledgers == null)
            {
                return NotFound();
            }
